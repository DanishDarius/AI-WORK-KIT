...
# Le lexique complet de l'IA : 50 notions à connaître pour comprendre ce que tu utilises
...
- Ton premier essai : la preuve dans la fiche
- Trois prompts pour passer à l'usage
- À toi de nommer : dix situations corrigées
- Garde les repères
...
À la fin de ce guide, tu pourras expliquer la différence entre contexte et mémoire, reconnaître un workflow ou une logique d'agent, et repérer une affirmation qui dépasse ce que prouvent réellement les faits. Le guide comprend un lexique consultable, quatre situations décodées, un exercice sur documents, trois prompts et dix questions corrigées.

Aucun code ni abonnement particulier n'est nécessaire pour lire et faire les exercices ; pour tester les prompts, utilise l'assistant conversationnel auquel tu as déjà accès. Les exemples restent pédagogiques, ce ne sont pas des résultats garantis dans ton propre compte. Apprendre ces mots pose une base solide ; la maîtrise vient ensuite de la pratique.
...
**1. Tu fournis — Prompt + contexte.** Une tâche, des consignes, et les informations utiles.

**2. Le système traite — Modèle → inférence.** Le modèle utilise les entrées disponibles pour produire une sortie.

**3. Tu contrôles — Réponse + preuves.** Tu vérifies les faits et décides de l'usage du résultat.
...
Ce que tu sais réellement : le PDF a été fourni à l'application. Son contenu peut entrer dans le contexte, être consulté par un outil, ou être récupéré par extraits — cela ne démontre en rien une modification des paramètres du modèle. La politique de données du service reste une question séparée.

Le bon contrôle : demande quel passage a été lu, et vérifie la politique applicable à ton espace.
...
## 04 — Ton premier essai : la preuve dans la fiche

Prends cinq minutes et travaille d'abord sur papier. Voici un dossier fictif volontairement court, pour distinguer consigne, contexte et information manquante.
...
Exercice :
1. Écris une consigne pour obtenir un brouillon de présentation de 60 à 80 mots. Précise le public et les limites.
2. Liste les faits utilisables et les informations qui doivent rester non renseignées.
3. Examine cette phrase proposée : « Rejoins notre atelier samedi à 18 h, il ne reste que deux places. » Repère ce qui ne vient pas du document.
4. Écris une version courte que tu pourrais faire valider avant diffusion.

Pour refaire cet exercice dans un assistant, copie le Document A et utilise le Prompt 2 ci-dessous. Si l'outil ajoute un détail non présent dans la fiche, retrouve la phrase fautive, indique la source manquante et fais reprendre le passage. Vérifie toujours le nouveau brouillon contre la fiche d'origine, pas seulement contre la promesse de l'assistant de l'avoir corrigé.

---

## 05 — Trois prompts pour passer à l'usage

Remplace les champs entre crochets avant de coller une consigne. Utilise des extraits que tu peux partager, sans données confidentielles inutiles. Ces prompts structurent un travail ; ils n'activent pas automatiquement une mémoire, un accès web ou un agent.

**Prompt 1 — Traduire le jargon de mon projet :**
> Aide-moi à comprendre ce texte sur l'IA, en français simple.
> Mon activité et mon niveau : [précise].
> Le texte à décoder : [colle le passage].
> La décision que je dois prendre : [précise ou indique « comprendre seulement »].
...
> Tâche et public : [précise]. Format attendu : [précise].
> Documents utilisables : [colle des extraits courts ou joins les fichiers autorisés].
...
> Mon activité : [précise]. Mon niveau : [débutant ou notions déjà connues].
> Référence : [colle les définitions du lexique de ce guide].
...
Le bon signe : tu réponds avant de lire l'explication, et tu peux donner un exemple différent. Si le modèle livre toutes les réponses d'un coup, rappelle « une situation à la fois, attends ma réponse ».

---

## 06 — À toi de nommer, puis de justifier

Réponds sans relire le lexique, puis vérifie ta réponse. Un choix juste avec une explication hésitante indique une notion à retravailler — refais ensuite un exemple dans ton propre métier.

1. Tu demandes un résumé d'un paragraphe. Quel processus produit la réponse ? → **L'inférence**
2. Tu ajoutes le public, le prix et la durée à ta consigne. Qu'apportes-tu ? → **Du contexte pour cette demande**
3. Une application retrouve deux extraits dans une base, puis les donne au modèle pour répondre. Quelle approche reconnais-tu ? → **Du RAG**
4. Un outil lit une photo et ta question écrite. Que peux-tu en déduire ? → **Il traite au moins deux modalités en entrée**
5. Le logiciel suit toujours formulaire → brouillon → validation, aucun modèle ne choisit les étapes. Comment le décris-tu ? → **Un workflow**
6. Un brouillon annonce « accueil à 18 h », alors que les documents n'indiquent aucun horaire. Que fais-tu ? → **Je marque l'horaire non renseigné et je vérifie auprès de la source**
7. Un service connecte ton assistant à des outils grâce à un protocole commun. De quelle notion parle-t-on ? → **MCP**
...
10. Avant de changer de modèle, tu définis des cas et des critères puis compares les résultats. Quelle pratique utilises-tu ? → **Des évaluations**
...
Pour continuer à progresser, choisis un outil que tu utilises déjà et décris une de ses fonctions en cinq lignes : entrée, traitement, sortie, contrôle et limites. N'utilise que les termes dont tu peux expliquer le rôle. Compare ensuite ton schéma à la documentation officielle du produit.
...
**Un mot devient vraiment utile quand tu peux l'expliquer avec un exemple.**

Prends une vraie situation de ton quotidien professionnel, nomme ce qui s'y passe parmi ces 50 notions, et identifie ce qui te permet de le vérifier. C'est cette habitude — nommer puis vérifier — qui transforme un vocabulaire flou en compréhension solide.
