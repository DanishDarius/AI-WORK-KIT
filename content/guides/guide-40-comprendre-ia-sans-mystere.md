...
- À toi de jouer : comprendre, puis décider
- Pour vérifier et approfondir

## Introduction

Tu utilises peut-être déjà une IA pour filtrer tes messages, traduire une phrase ou trouver une photo. Ici, tu vas comprendre ce qui se passe réellement derrière ces usages du quotidien. Un simple crayon suffit pour les exercices. Compte environ 20 minutes avec la pratique, davantage si tu souhaites explorer les sources citées.

**À la fin de ce guide, tu sauras :** reconnaître quelques usages de l'IA sans tout regrouper sous un seul nom de produit ; expliquer la différence entre entraîner un modèle et lui poser une question ; distinguer une réponse plausible d'un fait réellement établi.
...
*(Petit exercice de réflexion : une application envoie le même rappel tous les lundis à 9 h. Que peut-on en déduire ? Trois réponses possibles : elle utilise forcément une IA ; une règle programmée suffit pour cette fonction ; elle apprend tes habitudes à chaque rappel. La bonne réponse est la deuxième : une règle programmée simple suffit amplement à produire ce comportement, sans qu'aucune IA ne soit nécessaire. Le piège classique consiste à attribuer systématiquement à l'IA un comportement qu'une règle toute simple peut très bien expliquer.)*
...
3. **Utilisation.** Le modèle reçoit une nouvelle entrée (ta demande) et calcule une sortie : c'est ce qu'on appelle l'inférence. À toi ensuite d'évaluer le résultat obtenu.
...
La qualité du résultat dépend notamment des données utilisées, de la méthode d'entraînement, et de l'évaluation réalisée. Un modèle testé surtout sur un type de photos peut se révéler moins fiable sur un autre type. Un bon score moyen ne garantit jamais la réussite dans ton cas particulier.

*(À toi : explique l'entraînement avec un exemple de spam, en distinguant bien les exemples fournis, les régularités apprises, et l'utilisation finale sur un nouveau message.)*
...
Le contexte correspond aux informations disponibles pour produire la réponse : consignes, échanges précédents, documents joints ou résultats d'outils, selon le système utilisé. Ajouter un document change cette entrée pour cette conversation précise. Cela ne signifie absolument pas que tu réentraînes instantanément le modèle pour tous les autres utilisateurs.

Une réponse peut être claire, détaillée, et fausse en même temps. On appelle souvent « hallucination » une information inventée ou inexacte présentée comme si elle était établie. La phrase « je suis certain » prononcée par l'IA ne mesure en rien sa fiabilité réelle. Une source affichée par l'outil doit elle aussi être ouverte et vérifiée par toi-même avant d'être considérée comme fiable.

*(Petit exercice de réflexion : tu fournis le programme de ton atelier, et l'IA invente une adresse très précise qui n'existe pas. Que fais-tu ? Trois réponses possibles : je la garde, elle est précise ; je demande seulement à l'IA si elle est sûre ; je retire l'adresse et je la vérifie auprès de l'organisateur. La bonne réponse est la troisième : la précision apparente d'une information ne dit rien de son exactitude réelle, et demander à l'IA de confirmer sa propre erreur ne constitue jamais une vérification valable.)*

## À toi de jouer : comprendre, puis décider

Pour chaque situation suivante, écris toi-même l'entrée, la sortie attendue, et une erreur à surveiller particulièrement. N'ouvre aucun outil IA pour cet exercice : il s'agit d'entraîner ton propre raisonnement, pas de tester un modèle.
...
**Retiens ceci :** une IA transforme des entrées en sorties grâce à une méthode et, souvent, un modèle appris à partir de données. Ta compétence commence au moment où tu peux expliquer la tâche accomplie et contrôler ce qui en sort. Tu n'as absolument pas besoin de savoir programmer pour y parvenir.

**Avant de poursuivre :** sans relire ce guide, explique en deux phrases la différence entre entraînement et utilisation. Ajoute une raison pour laquelle une réponse bien écrite peut malgré tout être fausse.
