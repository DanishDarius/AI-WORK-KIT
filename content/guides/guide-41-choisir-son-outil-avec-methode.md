...
- À toi de jouer : justifie ton choix

## Introduction

Tu sais maintenant qu'un modèle peut classer, prévoir ou générer. Le choix suivant est éminemment pratique : quelle aide te faut-il exactement, et quelles informations peux-tu lui confier ? Compte environ 20 minutes avec l'exercice. Aucun achat ni inscription n'est nécessaire pour comparer les situations présentées ici.

**À la fin de ce guide, tu sauras :** choisir une famille d'outils à partir d'un résultat attendu précis ; distinguer modèle, application, recherche web et agent ; écarter un outil inutile ou trop risqué pour ta tâche.
...
Si tu disposes déjà d'un outil autorisé, essaie une tâche sans données sensibles. Utilise le même dossier pour comparer deux outils différents, avec exactement le même brief de départ. Une seule tentative isolée ne permet jamais de déclarer qu'un modèle est « le meilleur » dans l'absolu.
...
Tu peux tout à fait décider, au final, de ne pas utiliser l'IA du tout pour une tâche donnée. Un simple total dans un tableur, une information à demander directement à un collègue, ou une décision nécessitant l'avis d'un professionnel qualifié peuvent très bien être mieux traités autrement, sans passer par une IA.

## À toi de jouer : justifie ton choix

Sans ouvrir les corrections tout de suite, choisis l'aide utile pour ces trois demandes distinctes. Écris toi-même une entrée, une sortie, et un contrôle pour chacune :

- Rédiger l'invitation à l'atelier en 70 mots maximum.
- Montrer une courte scène imaginaire d'une table de réparation.
- Retrouver une règle précise dans le règlement validé de la salle.

**Ton livrable final :** une phrase justifiant l'outil choisi pour ta propre tâche, avec une limite explicitement assumée. Exemple : « J'utilise un assistant pour reformuler mes notes, mais je ne lui confie pas l'envoi et je vérifie les décisions dans le compte rendu d'origine. »

**Avant de poursuivre :** tu dois être capable de nommer précisément ce que ton outil doit lire, ce qu'il doit produire, et ce qu'il n'a explicitement pas le droit de faire à ta place.
