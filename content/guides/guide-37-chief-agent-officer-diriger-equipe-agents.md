...
Trois cours gratuits, une équipe de trois agents à cadrer, et une fiche d'évaluation d'une page pour documenter ton premier projet de délégation.
...
Tu peux commencer à apprendre le management d'agents avec un dossier de ton propre métier. Ton travail : décider ce qui mérite d'être délégué, répartir les tâches, contrôler les résultats, et intervenir quand une étape échoue.

Ce guide te fait produire une première preuve concrète de cette compétence. Tu trouveras trois cours gratuits, huit consignes prêtes à adapter, un exercice à trois agents, et une fiche d'évaluation d'une page. À la fin, tu pourras montrer un brief, les sorties obtenues, ta décision, et une reprise vérifiée.

**Le dossier que tu vas construire :** un brief (ce que tu délègues), deux versions (ce que tu corriges), une fiche (ce que tu prouves). Commence avec le cas fictif fourni plus bas, puis rejoue la méthode sur un vrai dossier, avec des données que tu es autorisé à utiliser.

## Comprendre le rôle avant de viser le titre

Un responsable d'agents organise un travail où des humains et des logiciels prennent en charge des étapes différentes. Dans ce guide, tu pilotes un petit processus : ses entrées, ses accès, ses résultats et son contrôle qualité.
...
Pour ton premier essai, prévois un créneau de 60 à 90 minutes comme repère de travail. Choisis une tâche que tu sais déjà juger toi-même : comparer trois propositions, préparer une note à partir de pièces, ou synthétiser un dossier. Évite de commencer par une décision que tu ne saurais pas vérifier toi-même après coup.

**Avant d'ouvrir la session :**

- Crée un dossier de démonstration séparé. Place-y uniquement les copies des pièces nécessaires, sans données sensibles.
- Définis trois ou quatre conditions de réussite mesurables. Un lecteur doit pouvoir refaire le contrôle lui-même.
...
Le processus suit une chaîne : recherche → rédaction → relecture → toi (la décision finale).

**Chief-recherche : établir les faits.** Il reçoit le brief et les pièces A, B, C. Il remet une matrice critère × option, avec preuve ou donnée manquante. Sa limite : il lit les pièces, mais ne choisit pas à ta place et ne complète aucun devis de lui-même. Tu vérifies que chaque cellule factuelle renvoie bien à une pièce précise, et que l'accès du lieu A reste marqué comme inconnu.
...
**Le chemin reproductible, avec un agent de code en ligne de commande.** Ouvre ton outil dans ton dossier de démonstration, avec un accès disponible. Tu peux lui demander de créer les sous-agents directement en français ; leurs définitions de projet se trouvent généralement dans un dossier de configuration dédié. Chaque sous-agent possède son propre contexte et ses propres outils.

1. Envoie la consigne de configuration avec les trois fiches de rôle ci-dessous, dans le même message.
...
**Une consigne de rôle n'est pas une barrière d'accès.** La liste d'outils limite les actions disponibles ; elle ne crée pas trois dossiers étanches. Les outils de lecture peuvent accéder aux fichiers autorisés par l'environnement dans son ensemble. Garde donc un dossier isolé contenant seulement les pièces utiles, et relis les permissions effectives. La session principale possède ses propres accès : ne lui accorde jamais une autorisation générale pour contourner un blocage rencontré ailleurs.
...
Le journal utile relie une demande à une exécution et à une sortie. Cherche les délégations, les résultats d'outils et les versions visibles dans ton interface. Garde les exports ou références qu'elle permet réellement de récupérer.
...
*(Exemple de ligne, fictif, à remplacer par ta propre observation :)* étape/entrée : relecture, note v2 et brief à 950 € ; constat : B dépasse le nouveau budget de 30 € ; preuve à conserver : pièce B à 980 €, nouveau brief à 950 €, passage de v2 qui explique l'impasse ; suite décidée par toi : demander un arbitrage sur le budget, sans réserver.

Si tu n'as pas de trace d'invocation, écris-le clairement. Si tu n'as pas mesuré le temps ou le coût, indique « non mesuré ». Ces absences donnent une prochaine étape d'apprentissage ; les combler avec des nombres inventés rendrait ton dossier moins crédible, pas plus impressionnant.
...
**Montre ce qui tient.** Présente un projet qui produit un livrable réellement utile à quelqu'un, avec ses limites assumées. Pour connecter un système métier ou automatiser une action externe, prépare séparément les accès et la validation avec la personne responsable de ce système.

**Ton récit en entretien, à compléter avec des faits réels :** « J'ai cadré [tâche] à partir de [pièces]. J'ai utilisé [mode réellement observé] pour répartir la recherche, la rédaction et la relecture. J'ai refusé v1 pour [motif], puis contrôlé [preuve] dans v2. Voici les sorties et ma fiche. La limite restante est [limite] ; mon prochain test est [test]. »

Sur ton CV, décris le projet et les responsabilités démontrées. Si tu as obtenu un badge de cours, indique son intitulé exact et son lien de vérification. Évite de présenter un simple badge d'achèvement comme une certification de « Chief Agent Officer ».

## À retenir

Ta valeur se voit dans tes décisions. Le moment où tu détectes une inconnue, refuses une conclusion et vérifies la reprise mérite autant de place dans ton portfolio que la note finale elle-même.
