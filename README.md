...
- [x] Authentification par lien magique (via Supabase Auth). Site URL et Redirect URLs configurés sur `https://ai-work-kit.parlonsads.com` (+ ancien domaine Vercel, wildcard preview Vercel et localhost pour le dev, conservés en secours).
...
- [x] Réception du lien magique : route `src/app/auth/confirm/route.ts` (vérification `token_hash`/`type` côté serveur) + middleware `src/lib/supabase/middleware.ts` (rafraîchissement de session). Un filet de sécurité côté client (`src/components/session-from-hash.tsx`) capture aussi la session quand elle arrive dans le fragment d'URL, puis recharge l'interface une fois la session écrite.
- [x] Reconnexion : page `/connexion` permettant à un compte existant de demander un nouveau lien par email, sans mot de passe et sans créer de compte hors parcours d'achat. Le header affiche automatiquement « Connexion » lorsqu'aucune session n'est présente.
...
- Le SMTP personnalisé étant en place, personnaliser le modèle "Invite user" (désormais éditable) pour qu'il pointe vers `/auth/confirm?token_hash={{ .TokenHash }}&type=invite&redirect_to={{ .RedirectTo }}` (route déjà prête) plutôt que le lien par défaut — plus robuste, y compris face aux clients mail qui pré-chargent les liens (ex. Outlook Safe Links) — et pour qu'il soit rédigé en français, à l'image d'AI WORK KIT.
